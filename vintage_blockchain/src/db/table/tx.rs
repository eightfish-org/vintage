...
macro_rules! define_tx_table {
    ($vis:vis ($table:ident, $table_r:ident, $table_w:ident) = ($tx:ty, $table_name:literal)) => {
        ...
        {
            pub fn check_tx_not_exists(&self, tx_id: &vintage_msg::TxId) -> anyhow::Result<()> {
            ...
            #[allow(dead_code)]
            pub fn check_txs_not_exist(&self, tx_ids: &[vintage_msg::TxId]) -> anyhow::Result<()> {
                for id in tx_ids {
            ...
            #[allow(dead_code)]
            pub fn get_tx(&self, tx_id: &vintage_msg::TxId) -> anyhow::Result<$tx> {
        ...
        impl<'db, 'txn> $table_w<'db, 'txn> {
            pub fn insert_tx(&mut self, tx_id: &vintage_msg::TxId, tx: &$tx) -> anyhow::Result<()> {
